...
namespace StarForce
{
    public class SettingForm : UGuiForm
    {
        ...
        //旁白显示
        [SerializeField]
        private Toggle m_UIAsideVoiceMuteToggle = null;

        [SerializeField]
        private Slider m_UITextSpeedSlider = null;


        //对话
        [SerializeField]
        private Toggle m_UIDialogVoiceMuteToggle = null;

        [SerializeField]
        private Slider m_UIDialogVoiceVolumeSlider = null;

        [SerializeField]
        private Slider m_UIDialogTextSpeedSlider = null;
        ...
        //旁白-文字读取速度
        public void OnUIVoiceAsideTextSpeedChanged(float speed)
        {
            Log.Debug(speed);
            GameEntry.Setting.SetFloat(Constant.Setting.UIAsideTextSpeed, speed);
        }

        //对话-语音
        public void OnSpeakMuteChanged(bool isOn)
        {

            Log.Debug(isOn);

            GameEntry.Setting.SetBool(Constant.Setting.UISpeakVolumeToggle, isOn);

            float volume = isOn ? GameEntry.Setting.GetFloat(Constant.Setting.UISpeakVolume) : 0;
            GameEntry.Event.Fire(this, TTSAudioVolumeChangeArgs.Create(volume));
            if (!isOn)
            {
                GameEntry.TTS.StopPlay();
            }
            m_UIDialogVoiceVolumeSlider.gameObject.SetActive(isOn);
        }

        public void OnUIVoiceSpeakVolumeChanged(float volume)
        {
            GameEntry.Setting.SetFloat(Constant.Setting.UISpeakVolume, volume);
            GameEntry.Event.Fire(this, TTSAudioVolumeChangeArgs.Create(volume));
        }
        //对话-文字读取速度

        public void OnUIVoiceSpeakTextSpeedChanged(float _speed)
        {
            GameEntry.Setting.SetFloat(Constant.Setting.UISpeakTextSpeed, _speed);
        }
        ...
        public void OnSubmitButtonClick()
        {
            ...
            {
                //旁白
                GameEntry.Setting.GetFloat(Constant.Setting.UIAsideTextSpeed,m_UITextSpeedSlider.value);

                //对话
                GameEntry.Setting.SetFloat(Constant.Setting.UISpeakVolume, m_UIDialogVoiceVolumeSlider.value);
                GameEntry.Setting.GetFloat(Constant.Setting.UISpeakTextSpeed, m_UIDialogTextSpeedSlider.value);
                Close();
                return;
            }
...
#endif
        {
            ...
            //旁白
            m_UITextSpeedSlider.value = GameEntry.Setting.GetFloat(Constant.Setting.UIAsideTextSpeed);

            //对话
            m_UIDialogVoiceMuteToggle.isOn = GameEntry.Setting.GetBool(Constant.Setting.UISpeakVolumeToggle); ;
            m_UIDialogVoiceVolumeSlider.value = GameEntry.Setting.GetFloat(Constant.Setting.UISpeakVolume); 
            m_UIDialogTextSpeedSlider.value = GameEntry.Setting.GetFloat(Constant.Setting.UISpeakTextSpeed); ;
